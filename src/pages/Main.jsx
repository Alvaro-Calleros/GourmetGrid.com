...
export default function App({ onLogout, username }) {
  ...
  const searchMealByName = async () => {
    if (!query.trim())
      return showMessage("Por favor, introduce un nombre para buscar.", "info");
  ...
  const getRandomMeals = useCallback(async () => {
    ...
    try {
      ...
      const allMeals = results.map((r) => r.meals && r.meals[0]).filter(Boolean);
      ...
      if (!uniqueMeals.length)
        showMessage("No se encontraron recetas iniciales.", "info");
    } catch (err) {
      showMessage(`Error al cargar recetas iniciales: ${err.message}`, "error");
    } finally {
      setLoading(false);
    }
  }, [setLoading, setMeals, setMessage, setActiveView, setCurrentPage, showMessage]);
    ...
    <>
      <div className="app-container">
        ...
        <div className="filters-container">
          ...
          <AreaFilter onSelectArea={filterByArea} API_BASE_URL={API_BASE_URL} />
        </div>
        ...
        {activeView === "favorites" ? (
          <FavoritesView getMealDetailsById={getMealDetailsById} username={username} />
        ) : (
          <>
            {activeFilter && (
        ...
        <button className="btn button-logout" onClick={onLogout}>
          Cerrar Sesion
        </button>
      </div>
    </>
